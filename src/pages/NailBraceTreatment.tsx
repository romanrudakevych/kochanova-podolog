...
const NailBraceTreatment = () => {
  const { t } = useTranslation();

  useEffect(() => {
    document.title = t("biomechanicalPage.metaTitle");
    const desc = document.querySelector('meta[name="description"]');
    desc?.setAttribute("content", t("biomechanicalPage.metaDescription"));
      ...
      <main className="pt-24 pb-16">
          ...
          <div className="container mx-auto px-6 py-16">
            ...
            >
              <span className="text-primary text-sm font-semibold tracking-wider uppercase">
                {t("biomechanicalPage.eyebrow")}
              </span>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground mt-3 text-glow">
                {t("biomechanicalPage.title")}
              </h1>
              <p className="text-muted-foreground mt-6 text-lg leading-relaxed">{t("biomechanicalPage.subtitle")}</p>
              <div className="glass-panel p-2 md:p-3 mt-10 max-w-2xl mx-auto">
                <img
                  src={nehtoveRovnatkoPhoto}
                  alt={t("biomechanicalPage.photo1.alt")}
        ...
        <section className="container mx-auto px-6 py-12">
          <div className="grid lg:grid-cols-2 gap-12 items-start max-w-6xl mx-auto">
            ...
            >
              <p>{t("biomechanicalPage.p1")}</p>
              <p>{t("biomechanicalPage.p2")}</p>
              <WhenToConsiderBlock pageKey="biomechanicalPage" />
            </motion.div>
            ...
            >
              <h2 className="text-xl font-semibold text-foreground mb-5">{t("biomechanicalPage.includesTitle")}</h2>
              <ul className="space-y-4">
                {includeKeys.map((key) => (
                  <li key={key} className="flex items-start gap-3">
                    <CheckCircle2 className="h-5 w-5 text-primary shrink-0 mt-0.5" aria-hidden />
                    <span className="text-sm text-muted-foreground leading-relaxed">
                      {t(`biomechanicalPage.includes.${key}`)}
                    </span>
                  </li>
                ))}
              </ul>
              <div className="mt-6 pt-6 border-t border-border/40">
                <span className="block text-sm text-muted-foreground mb-3">{t("biomechanicalPage.priceLabel")}</span>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-2xl sm:text-3xl font-bold text-primary">{t("biomechanicalPage.price")}</span>
                  <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
                    <Clock className="h-4 w-4" aria-hidden />
                    {t("services.biomechanical.duration")}
                  </span>
                </div>
              </div>
            </motion.div>
...
export default NailBraceTreatment;
