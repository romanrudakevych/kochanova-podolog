...
const PodologicConsultation = () => {
  const { t } = useTranslation();

  useEffect(() => {
    document.title = t("consultationPage.metaTitle");
    const desc = document.querySelector('meta[name="description"]');
    desc?.setAttribute("content", t("consultationPage.metaDescription"));
      ...
      <main className="pt-24 pb-16">
        <section className="relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 py-16">
            ...
            >
              ...
              <p className="text-muted-foreground mt-6 text-lg leading-relaxed">{t("consultationPage.subtitle")}</p>
              <div className="glass-panel p-2 md:p-3 mt-10 max-w-2xl mx-auto">
                <img
                  src={consultationPhoto1}
        ...
        <section className="container mx-auto px-6 py-12">
          <div className="grid lg:grid-cols-2 gap-12 items-start max-w-6xl mx-auto">
            ...
            >
              <h2 className="text-xl font-semibold text-foreground mb-5">{t("consultationPage.includesTitle")}</h2>
              <ul className="space-y-4">
                {includeKeys.map((key) => (
                  <li key={key} className="flex items-start gap-3">
                    <CheckCircle2 className="h-5 w-5 text-primary shrink-0 mt-0.5" aria-hidden />
                    <span className="text-sm text-muted-foreground leading-relaxed">
                      {t(`consultationPage.includes.${key}`)}
                    </span>
                  </li>
                ))}
              </ul>
              <div className="mt-6 pt-6 border-t border-border/40">
                <span className="block text-sm text-muted-foreground mb-3">{t("consultationPage.priceLabel")}</span>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-2xl sm:text-3xl font-bold text-primary">{t("consultationPage.price")}</span>
...
export default PodologicConsultation;
