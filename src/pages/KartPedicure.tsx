...
const KartPedicure = () => {
  ...
  useEffect(() => {
    document.title = t("kartPage.metaTitle");
    const desc = document.querySelector('meta[name="description"]');
    desc?.setAttribute("content", t("kartPage.metaDescription"));
      ...
      <main className="pt-24 pb-16">
        <section className="relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 py-16">
            ...
            >
              ...
              <p className="text-muted-foreground mt-6 text-lg leading-relaxed">{t("kartPage.subtitle")}</p>
              <div className="glass-panel p-2 md:p-3 mt-10 max-w-2xl mx-auto">
                <img
                  src={kartPedikuraPhoto}
                  alt={t("kartPage.photo1.alt")}
        ...
        <section className="container mx-auto px-6 py-12">
          <div className="grid lg:grid-cols-2 gap-12 items-start max-w-6xl mx-auto">
            ...
            >
              <h2 className="text-xl font-semibold text-foreground mb-5">{t("kartPage.includesTitle")}</h2>
              <ul className="space-y-4">
                {includeKeys.map((key) => (
                  <li key={key} className="flex items-start gap-3">
                    <CheckCircle2 className="h-5 w-5 text-primary shrink-0 mt-0.5" aria-hidden />
                    <span className="text-sm text-muted-foreground leading-relaxed">
                      {t(`kartPage.includes.${key}`)}
                    </span>
                  </li>
                ))}
              </ul>
              <div className="mt-6 pt-6 border-t border-border/40">
                <span className="block text-sm text-muted-foreground mb-3">{t("kartPage.priceLabel")}</span>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-2xl sm:text-3xl font-bold text-primary">{t("kartPage.price")}</span>
                  <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
                    <Clock className="h-4 w-4" aria-hidden />
                    {t("services.kart.duration")}
                  </span>
                </div>
              </div>
            </motion.div>
